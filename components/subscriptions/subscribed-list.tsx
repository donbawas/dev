        ...
        <TabsContent key={cat} value={cat} className="mt-4">
          <div className="flex flex-col divide-y divide-border rounded-xl border border-border overflow-hidden">
            {byCategory[cat].map((topic) => (
              <div
                key={topic.id}
                className="flex items-center gap-3 bg-card px-4 py-3 hover:bg-muted/30 transition-colors"
              >
                ...
                {topic.last_update_at ? (
                  ...
                  <span className="hidden shrink-0 text-[11px] text-muted-foreground/40 lg:block">
                    no updates
                  </span>
                )}
